function [spectra, normSpectra] = calc_spectra_fitting(f, fit_parameters)
% calculate different spectra

params = utils.loadParameters_new;

tau = fit_parameters(1);
kappa = fit_parameters(2);
w_f = fit_parameters(3);
...
Q = params.C_z*(Y1*(params.eta + tau^(-1)) - ...
Y2*params.C_z*(params.eta - (tau^(-1))*(params.beta - 2)) + ...
    (params.D/params.rho_f)*(Y1 - params.V_0*Y2));
R = params.C_z*(params.D/params.rho_f)*(Y1*(params.eta + tau^(-1)) - ...
    Y2*params.C_z*(params.eta - (tau^(-1))*(params.beta - 2)));
...
spectra.P2 = 1./((-w.^2 + kappa^2/4 + w_f^2).^2 + kappa^2*w.^2);
spectra.P3 = 1./(w.^2 + (params.eta + tau^(-1)).^2);
